...
import { UPLOAD_SOURCES, type UploadField, type UploadFiles } from "@/constants/uploads";
...
export function FileUploadSection({
  ...
  return (
    <section aria-labelledby="upload-heading" className="space-y-5">
      <div>
        <p className="text-xs font-semibold tracking-wider text-primary uppercase">Input files</p>
        <h2 id="upload-heading" className="mt-1 text-xl font-semibold tracking-tight">
          Choose the three source exports
        </h2>
        <p className="mt-1 max-w-2xl text-sm leading-6 text-muted-foreground">
          Select one CSV for each source. File contents are validated by the reconciliation service.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-4 border-t pt-5 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex max-w-xl items-start gap-2 text-sm text-muted-foreground">
          ...
          <p>Files are processed for this request and are not stored by the current service.</p>
        </div>
        <Button
          type="button"
          size="lg"

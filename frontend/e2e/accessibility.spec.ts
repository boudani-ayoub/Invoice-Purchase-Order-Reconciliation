...
test("initial upload state has no serious or critical axe violations", async ({
  page,
}) => {
  await page.goto("/");
  await expect(page.getByLabel("Choose purchase orders")).toBeVisible();
  await expectNoHighImpactViolations(page);
});

test("ready state has no serious or critical axe violations", async ({
  page,
}) => {
  await page.goto("/");
...
test("successful result state has no serious or critical axe violations", async ({
  page,
}) => {
  await page.goto("/");
  await selectSourceFiles(page);

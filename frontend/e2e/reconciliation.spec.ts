...
test("reconciles the sample exports and supports inspection and reset", async ({ page }) => {
  const response = await page.goto("/");

  expect(response?.headers()["content-security-policy"]).toContain("frame-ancestors 'none'");
  expect(response?.headers()["x-content-type-options"]).toBe("nosniff");
  await expect(page.getByRole("heading", { name: "Choose the three source exports" })).toBeVisible();
  ...
  const resultHeading = page.getByRole("heading", { name: "Reconciliation results" });
  ...
  await page.getByRole("searchbox", { name: "Search reconciliation lines" }).fill("INV-001");
  await expect(page.getByText("Showing 1 of 17 lines")).toBeVisible();
  await expect(page.getByText("INV-001", { exact: true })).toBeVisible();
  await page.getByRole("searchbox", { name: "Search reconciliation lines" }).fill("");
...
test("shows structured validation details from the real API", async ({ page }, testInfo) => {
...
test("accepts a near-limit file without allowing duplicate submission", async ({ page }, testInfo) => {
  test.slow();
...
test("reports an oversized file and succeeds after replacement", async ({ page }, testInfo) => {
  test.slow();
  ...
  const alert = page.getByRole("alert").filter({ hasText: "file is too large" });
  await expect(alert).toContainText("Purchase orders file is too large");
  await expect(alert).toContainText("10.0 MB");

  await page.getByLabel("Choose purchase orders").setInputFiles(SAMPLE_FILES.purchase_orders);
  await expect(alert).toBeHidden();
  await page.getByRole("button", { name: "Run reconciliation" }).click();
  await expect(page.getByRole("heading", { name: "Reconciliation results" })).toBeVisible();
});

test("recovers after the reconciliation service is unavailable", async ({ page }) => {
  await page.route(RECONCILIATION_URL, (route) => route.abort("connectionrefused"));
  await page.goto("/");
  await selectSourceFiles(page);
  await page.getByRole("button", { name: "Run reconciliation" }).click();
  ...
  await expect(alert).toContainText("The reconciliation service could not be reached");
  await page.unroute(RECONCILIATION_URL);
  await page.getByRole("button", { name: "Try again" }).click();
  await expect(page.getByRole("heading", { name: "Reconciliation results" })).toBeVisible();
});

test("keeps long Unicode identifiers usable on a mobile viewport", async ({ page }, testInfo) => {

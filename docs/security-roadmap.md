...
model. The current service is stateless and remains intended for trusted environments.

| Control area | Current evidence / boundary | Planned work |
| --- | --- | --- |
| Authentication | No credentials, login, or public identity API | Phase 2: credential policy, hashing, registration, verification, login throttling |
| Session management | No session cookies or tokens | Phase 2: server-side session choice, secure cookies, expiry, rotation, revocation |
| Password recovery | No recovery flow | Phase 2: bounded single-use tokens, enumeration resistance, session revocation |
| Authorization | Persistent HTTP routes absent | Phase 2: authenticated membership, active organizations, role/action permissions, negative tests |
| Tenant isolation | Composite organization FKs; ENABLE/FORCE RLS; missing context denied; pool reuse tests | Phase 2: derive transaction context from verified identity and membership; never trust a header as proof |
| CSRF | Stateless uploads have no saved-data side effect | Phase 2: token/origin controls appropriate to chosen session cookies; test every state-changing route |
| Rate limiting | No application rate limiter | Phase 2/9: login, reset, upload, and resource budgets at trusted deployment boundaries |
...
| Logging and audit | Generic HTTP errors; no actor-aware audit writes | Phase 2/3: actor, organization, event, resource, UTC timestamp, correlation ID, safe metadata; no content/secrets |
| Secrets | Private DATABASE_URL; no database imports in HTTP or browser code | Phase 2/9: secret manager, rotation, scoped service credentials, sanitized operational errors |
| Database least privilege | Separate schema owner and runtime roles; no BYPASSRLS/superuser/table ownership for runtime | Phase 2/9: verify deployed grants and role memberships, network restrictions, TLS |
...
| Backup/restore | No customer data is persisted through the product | Phase 3/9: encrypted backups, least privilege, tested restores, recovery objectives |
...
1. **Phase 1 — current:** four analysis modes and optional PostgreSQL foundation.
2. **Phase 2:** identity, organizations, secure sessions, tenant authorization, protected routes.
...
RLS protects configured database transactions, not anonymous HTTP requests. A process able to

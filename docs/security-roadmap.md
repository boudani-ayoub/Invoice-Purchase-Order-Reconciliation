...
| Authorization | Explicit role maps for runs, workflow, dashboard, and organization administration; every admin operation rechecks active membership; tenant resources use `404` and insufficient roles use `403` | Review future Phase 7 inventory permissions without widening MEMBER/AP_MANAGER implicitly |
...
| Logging and audit | Server request UUID; append-only run/workflow/governance events; ORG_ADMIN-only bounded merged timeline; workflow bodies and invitation tokens excluded | Phase 9: correlation/alerts, tamper-evident export if required, and DB/proxy log controls |
...
7. **Phase 7 — next:** inventory foundation and explicit stock movements.
